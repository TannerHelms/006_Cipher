...
<?php }
	...
	*/
	...
	WHERE id=?"))
	{
	$stmt->bind_param("sssi", $firstname, $lastname, $email, $website, $comments, $id);
	$stmt->execute();
	$stmt->close();
	...
	if($stmt = $conn->prepare("SELECT id, firstname, lastname, email, website, comments FROM MyGuests WHERE id=?"))
	{
	$stmt->bind_param("i", $id);
	$stmt->execute();

	$stmt->bind_result($id, $firstname, $lastname, $email, $website, $comments);
	$stmt->fetch();
	...
	*/
	...
	{
	$stmt->bind_param("sss", $firstname, $lastname, $email, $website, $comments);
	$stmt->execute();
	$stmt->close();
	}
	// show an error if the query has an error
